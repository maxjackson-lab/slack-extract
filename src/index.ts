...
class SlackExtractorApp {
  ...
  private analyzer: UnifiedAnalyzer | null = null;
   ...
   */
  private findLatestCsvFile(): string | null {
    const fs = require('fs');
    ...
    try {
      ...
      const files = fs.readdirSync(exportsDir)
        .filter((file: string) => file.startsWith('slack-data-export_') && file.endsWith('.csv'))
        .map((file: string) => ({
          name: file,
  ...
  private setupAnalyzer(): void {
    ...
    if (openaiApiKey && gammaApiKey) {
      ...
      this.analyzer = new UnifiedAnalyzer(openaiApiKey, gammaApiKey, analysisConfig);
